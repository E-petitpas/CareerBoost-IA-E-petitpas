...
async function rejectCompany(companyName) {
  ...
  try {
    ...
    console.log(`✅ Entreprise "${company.name}" rejetée avec succès !`);
...
if (!companyName) {
  console.log('❌ Usage: npm run reject-company "Nom de l\'entreprise"');
  console.log('   Exemple: npm run reject-company "Favoris"');
  process.exit(1);
}
